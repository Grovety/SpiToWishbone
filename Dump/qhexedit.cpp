...
QHexEdit::QHexEdit(QWidget *parent) : QAbstractScrollArea(parent)
{
    ...
    m_saveAct = new QAction(QIcon(":/Icons/icons/save.png"), tr("&Save"), this);
    m_saveAct->setShortcuts(QKeySequence::Save);
    m_saveAct->setStatusTip(tr("Save the document to disk"));
    connect(m_saveAct, &QAction::triggered, this, &QHexEdit::OnSaveData);

    m_openAct = new QAction(QIcon(":/Icons/icons/open.png"), tr("&Load from cursor..."), this);
    m_openAct->setShortcuts(QKeySequence::Open);
    m_openAct->setStatusTip(tr("Open an existing file"));
    connect(m_openAct, &QAction::triggered, this, &QHexEdit::OnOpenData);
}
...
void QHexEdit::OnOpenData()
{
    QString fileName = QFileDialog::getOpenFileName(this,"Select File Name",m_defauiltFileName,"Binary Files (*.bin);;All Files (*.*)");
    if (fileName.isEmpty())
    {
        return;
    }
    ...
    {
        if (m_curAddress + file.size()>data().size())
        {
            QMessageBox::about(this,"Error","File size is larger than this container!");
            return;
        }
        pos();
        file.read((char*)_data.constData() + m_curAddress,file.size());
        file.close();
        _bData.setData(_data);
        setData(_bData);
    }
...
void QHexEdit::OnSaveData()
{
    QString fileName = QFileDialog::getSaveFileName(this,"Select File Name",m_defauiltFileName,"Binary Files (*.bin);;All Files (*.*)");
    if (fileName.isEmpty())
    {
        return;
    }
